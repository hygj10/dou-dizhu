...
class Group:
    ...
    def play_card(self, card):
        self.card_on_table.append(card)
        return
    
    def current_card(self):
        return self.card_on_table[-1]
    
    def new_round(self):
        return self.card_on_table[-1] == [] and self.card_on_table[-2] == []
...
######        
    def game_connect(self, me, peer):
        ...
        peer_in_group, group_key = self.find_group(peer)
        if peer_in_group == True:
            print(peer, "is playing already, connect!")
            self.chat_grps[group_key].append(me)
            self.members[me] = S_TALKING
        else:
            # otherwise, create a new group
    ...
    def set_card_decks(self, name, deck):
        self.card_decks[name] = deck
        return
